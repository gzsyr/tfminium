...
class TestTfqHuaTiDetail(TestBase):
    ...
    def test_toupiao_投票(self):
        ...
        if self.page.element_is_exists('view[class="vote--vote-btn"]', max_timeout=5):
            # 投票按钮存在，进行选项选择 并 投票
            self.page.get_element('view[class="vote--item"]').tap()
            self.page.get_element('view[class="vote--vote-btn"]').tap()
