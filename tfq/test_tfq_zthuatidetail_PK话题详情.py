...
class TestTfqHuaTiDetail(TestBase):

    """
    V6.26.X：1004926 PK话题详情页
    """
    def setUp(self) -> None:
        # self.huatiid = 20211
        self.page_name = f"/page/taofangquan/tieziDetail/tieziDetail?city=qz&postsid={self.pkhuatiid}"
        self.switch = False
    ...
    def test_PK_投票(self):
        """
        PK话题详情页，对运营刚刚发的投票，进行选项PK
        """
        print(self.page.query['postsid'])
        try:
            # 支持按钮存在，进行选择
            self.find_element('view[class="PK--button"]', inner_text='支持').tap()
            self.get_screenshot()
        except minium.MiniElementNotFoundError:
            self.get_screenshot('test_PK_投票无支持按钮')


    # 话题与帖子共用一个页面，下面用例可以省略
